...
ssize_t IMUSerialPort::write(const uint8_t* data, size_t length) {
    if (fd_ < 0) {
        if (logger_) logger_->error("write called on closed serial port: {}", interface_);
        return -1;
    }
    size_t total = 0;
    while (total < length) {
        ...
        if (n < 0) {
            if (errno == EINTR) continue;
            if (logger_) logger_->error("serial write error: {}", strerror(errno));
