...
void InferenceNode::setup_model(std::unique_ptr<ModelContext>& ctx, std::string model_path, int input_size){
    ...
    for (size_t i = 0; i < ctx->num_outputs; i++) {
        ...
        // Resolve a dynamic batch dimension (e.g. mevius2's [-1, 12] output) to
        // 1, mirroring the input-shape handling above. Models with a static
        // batch dim (e.g. the humanoid policies) are unaffected.
        if (!ctx->output_shape.empty() && ctx->output_shape[0] == -1) {
            ctx->output_shape[0] = 1;
        }
    }
        ...
        try {
...

            {
                ...
                for (int i = 0; i < policy.ctx->output_buffer.size(); i++) {
                    ...
                    const float sc = (i < static_cast<int>(action_scale_.size())) ? action_scale_[i] : action_scale_.back();
                    act_[usd2urdf_[i]] = policy.ctx->output_buffer[i] * dof_sym_sign_[i] * sc + joint_default_angle_[usd2urdf_[i]];
                }
