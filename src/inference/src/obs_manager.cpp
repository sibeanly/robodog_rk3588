...
void InferenceNode::get_dof_pos_obs(std::vector<float>& segment) {
    ...
    for (int i = 0; i < joint_num_; i++) {
        segment[i] = (joint_pos_buffer_[usd2urdf_[i]] - joint_default_angle_[usd2urdf_[i]])
                     * obs_scales_dof_pos_ * dof_sym_sign_[usd2urdf_[i]];
    }
...
void InferenceNode::get_dof_vel_obs(std::vector<float>& segment) {
    ...
    for (int i = 0; i < joint_num_; i++) {
        segment[i] = joint_vel_buffer_[usd2urdf_[i]] * obs_scales_dof_vel_ * dof_sym_sign_[usd2urdf_[i]];
    }
}
...
void InferenceNode::get_is_standing_obs(std::vector<float>& segment) {
    std::unique_lock<std::mutex> lock(cmd_mutex_);
    const float cmd_norm = std::sqrt(cmd_vel_[0] * cmd_vel_[0]
